...
class WallFollower(Node):
    ...
    def clbk_laser(self, msg):
        ...
        bool_outer_corner = self.is_outer_corner()
        bool_inner_corner = self.is_inner_corner()
...
def main(args=None):
    ...
    rate = wall_follower_node.create_rate(hz)
    ...
    try:
        while rclpy.ok():
            ...
            # State Dispatcher
            if state_ == 0:
                msg = wall_follower_node.random_wandering()
            elif state_ == 1:
                msg = wall_follower_node.following_wall()
            elif state_ == 2:
                msg = rotating()
            else:
                wall_follower_node.get_logger().error('Unknown state!')

            wall_follower_node.pub_.publish(msg)
